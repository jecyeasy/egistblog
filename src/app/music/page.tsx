...
const Music: React.FC = () => {
  ...
  const { data, error, isLoading } = useSWR("/api/musicApi", fetcher, {
    revalidateOnFocus: false, // Disable re-fetching on focus
    refreshInterval: 0, // Disable automatic refetching
  });

  const handlePostClick = (id: number) => {
    router.push(`/moviesApi/${id}`); // Navigate to the dynamic blog post page
  };
  ...
  return (
    <div> 
      <div>
        <h1 className="text-3xl ml-[50px] font-bold">music</h1>
